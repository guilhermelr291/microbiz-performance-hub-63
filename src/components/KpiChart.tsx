  ...
  const dataWithGoals = data.map((item, index) => {
    if (goalValue) {
      ...
      const isAverageMetric =
        title.toLowerCase().includes('médio') ||
            ...
            >
              ...
              <Tooltip
                formatter={(value, name) => {
                  if (name === 'previous')
                    return [`${prefix}${value}${suffix}`, 'Período Anterior'];
                  if (name === 'current')
                    return [`${prefix}${value}${suffix}`, 'Período Atual'];
                  if (name === 'goal')
                    return [`${prefix}${value}${suffix}`, 'Meta'];
                    ...
                    return (
                      <div className="bg-background border border-border rounded-lg shadow-lg p-3 min-w-[200px]">
                        ...
                        {dataItem?.comparison !== undefined && (
                          <div
                            className={`flex items-center rounded-md px-2 py-1 text-xs font-semibold mb-2 ${
